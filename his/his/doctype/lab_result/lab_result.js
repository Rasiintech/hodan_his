...
frappe.ui.form.on('Lab Result', {
    ...
    refresh: function(frm) {
        ...
        // --- Helper Function to Open Print Format ---
        const print_report = (format_name) => {
            let url = `${frappe.urllib.get_base_url()}/printview?doctype=Lab%20Result&name=${frm.doc.name}&trigger_print=1&format=${format_name}&no_letterhead=0&letterhead=Logo&settings=%7B%7D&_lang=en-US`;
            window.open(url);
        };

        // --- 1. Print CBC Button ---
        frm.add_custom_button(__('Print CBC'), () => {
            print_report('CBC V2'); // Calls the print format named 'cbc'
        }, __('Print Options'));

        // --- 2. Print Stool Button ---
        frm.add_custom_button(__('Print Stool'), () => {
            print_report('Stool V2'); // Calls the print format named 'stool'
        }, __('Print Options'));

        // --- 3. Print Urine Button ---
        frm.add_custom_button(__('Print Urine'), () => {
            print_report('Urine V2'); // Calls the print format named 'Urine'
        }, __('Print Options'));

        frm.add_custom_button(__('Print Body Fluid'), () => {
            print_report('Body Fluid'); // Calls the print format named 'Urine'
        }, __('Print Options'));

        // --- 4. Print All (The Rest) Button ---
        frm.add_custom_button(__('Print Other'), () => {
            print_report('Print All V2'); // Calls the print format named 'print all'
        }, __('Print Options'));
