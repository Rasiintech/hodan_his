...
class ReceivablePayableReport(object):
	...
	def set_invoice_details(self, row):
		...
		if row.voucher_type == "Journal Entry":
			row.company = frappe.db.get_value("Journal Entry", row.voucher_no, "company")
			row.patient = frappe.db.get_value("Journal Entry", row.voucher_no, "patient")
			row.policy_number = frappe.db.get_value("Insurance Policy", {"policyholder":frappe.db.get_value("Journal Entry", row.voucher_no, "patient")}, "policy_number")
			row.patient_name = frappe.db.get_value("Journal Entry", row.voucher_no, "patient_name")
			row.sales_invoice = frappe.db.get_value("Journal Entry", row.voucher_no, "sales_invoice") or frappe.db.get_value("Journal Entry", row.voucher_no, "reference_invoice")
			row.dob = frappe.db.get_value("Patient", {"name":frappe.db.get_value("Journal Entry", row.voucher_no, "patient")}, "dob")
		if row.due_date:
	...
	def get_columns(self):
		...
		self.add_column("Patient ID", fieldname="patient", fieldtype="Data")
		self.add_column("Hospital Name", fieldname="company", fieldtype="Data", width=200)
		self.add_column("Patient Name", fieldname="patient_name", fieldtype="Data", width=200)
		self.add_column("Date of Birth", fieldname="dob", fieldtype="Data", width=200)
		self.add_column("Policy Number", fieldname="policy_number", fieldtype="Data", width=200)
		
		
		self.add_column("Invoice", fieldname="sales_invoice", fieldtype="Link", options="Sales Invoice")
		# if self.party_type == "Customer":
		# 	self.add_column(
		# 		_("Customer Contact"),
