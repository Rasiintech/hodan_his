...
def execute(filters=None):
    ...
    delivered_by_item = frappe.db.sql("""
        SELECT
            ...
            SUM(CASE WHEN se.docstatus = 1 THEN IFNULL(sed.amount, sed.basic_amount) ELSE 0 END) AS delivered_stock_amount
    ...
    delivered_map = {
        d.mri_row: {
            "stock_qty": flt(d.delivered_stock_qty),
            "stock_amount": flt(d.delivered_stock_amount),
        }
        for d in delivered_by_item
    }
    ...
    for r in base_rows:
        mr = mr_map.setdefault(r.mr_no, {
            ...
            "delivered_stock_amount": 0.0,
        })
        mr["item_count"] += 1
        mr["req_stock_qty"] += flt(r.req_stock_qty)
        mr["amount"] += flt(r.amount or 0)
        delivered = delivered_map.get(r.mri_row, {})
        mr["delivered_stock_qty"] += flt(delivered.get("stock_qty"))
        mr["delivered_stock_amount"] += flt(delivered.get("stock_amount"))
    ...
    for mr_no, v in sorted(mr_map.items(), key=lambda kv: (kv[1]["mr_date"], kv[0])):
        ...
        rows.append({
            ...
            "delivered_stock_amount": flt(v["delivered_stock_amount"], 2),
    ...
    for r in rows:
        ...
        totals["delivered_stock_amount"] += flt(r["delivered_stock_amount"])
    ...
    report_summary = [
        ...
        {"label": _("Delivered Stock Amount"),
         "value": frappe.utils.fmt_money(totals["delivered_stock_amount"]) if totals["delivered_stock_amount"] else "-",
         "indicator": "green"},
...
def get_columns():
    return [
        ...
        {"label": _("Delivered Stock Amount"), "fieldname": "delivered_stock_amount", "fieldtype": "Currency", "width": 170},
