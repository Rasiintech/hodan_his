...
EmergencyNurse = Class.extend(
	{
		...
		//  let fields = frappe.get_meta("Sales Order").fields
			...
			{title:"Consultant", field:"practitioner" ,  headerFilter:"input"},
		...
		tbldata.forEach(row => {
			...
			// }
			btnhml +=  `<button class='btn btn-primary ml-2' onclick = "open_vital('${row.patient}' , '${row.practitioner}')"> Vital Sings</button>`
			row['action'] = btnhml
			new_data.push(row)
		})
		// console.log(columns)
...
open_vital = function(patient , doctor){
	
	frappe.new_doc("Vital Signs", {"patient": patient , doctor : doctor})
