...
IPD = Class.extend(
	{
		...
		tbldata.forEach(row => {
			...
			if(row.clearance_status == "Cleared"){
				btnhml = `
				<button class='btn btn-success  ml-2' onclick = "discharge('${row.name}')"> Discharge</button>
				`

			}
			else if(row.clearance_status == "Uncleared"){
				btnhml = `
			<p class='btn btn-info ml-2' >Uncleared</p>
			...
			let btncancel =  `
			<button class='btn btn-danger ml-2' onclick = "cancel('${row.name}')"> Cancel</button>
			`

			
			row['action'] = btnhml + btncancel
			new_data.push(row)
		})
		// console.log(columns)
...
function cancel(name){
	...
	// alert(name)
		...
		() => {
			...
			frappe.call({
				method: "his.his.doctype.discharge_summary.discharge_summary.cancel_schedule", //dotted path to server method
				args: {
					'inpatient_record' : name
				},
				callback: function(r) {
				...
				frappe.show_alert({
				message:__('You have Cancelled Patient Succesfully'),
				indicator:'green',
