frappe.pages['pediatrics'].on_page_load = function (wrapper) {
	new IPD(wrapper)
}
...
IPD = Class.extend(
	{
		...
		setupdata_table: function (gr_ref) {
			...
			frappe.db.get_list('Inpatient Record', {
				fields: ['patient', 'patient_name', 'age', 'type', 'room', 'bed', 'admitted_datetime', 'medical_department', 'admission_practitioner', 'diagnose'],
				filters: {
					"status": 'Admitted',
					"type": "Pediatric"
				},
				...
				columns = [
					...
					{ title: "Medical Department", field: "medical_department", headerFilter: "input", formatter: durationformatter },
				...
				this.table.on("rowClick", function (e, rows) {
					...
					frappe.new_doc("Patient History", { patient: rows._row.data.patient, type: rows._row.data.type, consultant: rows._row.data.admission_practitioner })
...
<div id="ped" style = "min-width : 100%"></div>
