frappe.pages['profit-and-loss'].on_page_load = function(wrapper) {
	new profit_and_loss_dashboard(wrapper);
};

profit_and_loss_dashboard = Class.extend({
	init: function(wrapper) {
		...
		this.themeStorageKey = 'profit_and_loss_dashboard_theme';
	...
	load_dashboard: function() {
		...
		frappe.call({
			...
			args: {
				...
				previous_from_date: this.previous_from_date,
				previous_to_date: this.previous_to_date
			},
			callback: (r) => {
				...
				this.previous_from_date = data.previous_from_date || this.previous_from_date;
				this.previous_to_date = data.previous_to_date || this.previous_to_date;
	...
	render_dashboard: function(data, preserve_sidebar) {
		const $dashboard = $(frappe.render_template('profit_and_loss', data));
		if (preserve_sidebar) {
	...
	get_loading_context: function() {
		...
		return {
			...
			budget_variance_message: 'Loading account variance...',
			cash_flow: [],
			insights: [],
	...
	load_ai_insights: function(data) {
		...
		$target.html('<p class="ai-insight-placeholder"><i class="fa fa-spinner fa-spin slate-text"></i>Generating AI profit and loss insight...</p>');

		frappe.call({
			method: 'his.his.page.profit_and_loss.profit_and_loss.get_ai_insights',
			type: 'POST',
			args: {
				from_date: this.from_date,
				to_date: this.to_date,
	...
	bind_actions: function() {
		this.page.main.find('.accounts-nav [data-route]').on('click', (event) => {
			...
			if (route && route !== 'profit-and-loss') {
		...
		this.page.main.find('[data-budget-plan-link="1"]').on('click', (event) => {
			...
			frappe.route_options = {
				...
				period_start_date: this.from_date,
				period_end_date: this.to_date,
				periodicity: 'Yearly',
	...
	export_dashboard_csv: function() {
		...
		const fileName = `profit-and-loss-${fromDate}-to-${toDate}.csv`;
		...
		frappe.show_alert({
			message: __('Profit and loss dashboard export downloaded'),
			indicator: 'green'
		});
	},
	...
	open_print_window: function(mode) {
		...
		const title = mode === 'pdf' ? 'Profit and Loss Dashboard PDF Export' : 'Profit and Loss Dashboard Print View';
	...
	build_export_csv: function(data) {
		...
		addRow('Profit and Loss Dashboard Export');
		...
		addSectionTitle('Top Income Accounts');
		addRow('Account', 'Type', 'Income');
		(data.account_balances || []).forEach((row) => {
			addRow(row.account, row.type, row.balance);
		});
		addRow('Total', 'Top Income', data.account_balances_total || '');
		addBlankRow();

		addSectionTitle('Top Expense Accounts');
		addRow('Account', 'Entries', 'Expense');
		...
		addSectionTitle('Net Result by Period');
		addRow('Period', 'Status', 'Net Result');
		(data.top_supplier_balances || []).forEach((row) => {
			addRow(row.supplier, row.supplier_group, row.balance);
		});
		addRow('Total', 'All Periods', data.top_supplier_balances_total || '');
		addBlankRow();

		addSectionTitle('Account Variance vs Previous Period');
		if ((data.budget_variance || []).length) {
			addRow('Account', 'Current', 'Previous', 'Variance', 'Direction', 'Trend');
			...
			addRow(data.budget_variance_message || 'No profit and loss data available');
		}
		addBlankRow();
...
<body>
	...
	<div class="section-block">
		<h2>Top Income Accounts</h2>
		<table>
			<thead><tr><th>Account</th><th>Type</th><th>Income</th></tr></thead>
	...
	<div class="section-block">
		<h2>Top Expense Accounts</h2>
		<table>
			<thead><tr><th>Account</th><th>Entries</th><th>Expense</th></tr></thead>
			<tbody>${renderRows(data.unpaid_invoices, ['customer_group', 'customer_count', 'outstanding'])}</tbody>
		</table>
		<p><strong>Total:</strong> ${this.escape_html(String((data.unpaid_invoices_total || {}).customer_count || 0))} entries, ${this.escape_html((data.unpaid_invoices_total || {}).outstanding || '')}</p>
	</div>

	<div class="section-block">
		<h2>Net Result by Period</h2>
		<table>
			<thead><tr><th>Period</th><th>Status</th><th>Net Result</th></tr></thead>
	...
	<div class="section-block">
		<h2>Account Variance vs Previous Period</h2>
		${(data.budget_variance || []).length ? `
			<table>
				<thead><tr><th>Account</th><th>Current</th><th>Previous</th><th>Variance</th><th>Direction</th><th>Trend</th></tr></thead>
		...
		` : `<p>${this.escape_html(data.budget_variance_message || 'No profit and loss data available')}</p>`}
	</div>
	...
	show_theme_picker: function() {
		...
		const themes = [
			{ key: 'standard', label: 'Standard', description: 'Original profit and loss dashboard palette' },
	...
	show_date_filter: function() {
		...
		const defaultPreviousRange = this.get_previous_period_for_range(
			this.from_date || ranges.this_month.from_date,
			this.to_date || ranges.this_month.to_date
		);
		const $popover = $(`
			<div class="accounts-date-popover">
				<div class="accounts-date-popover-head">
					<strong>Comparison Dates</strong>
				...
				<div class="accounts-date-section">
					...
					<div class="accounts-date-fields">
						<label>
							<span>From</span>
							<input type="date" data-date-field="previous_from_date" value="${this.escape_attribute(this.previous_from_date || defaultPreviousRange.from_date)}">
						</label>
						<label>
							<span>To</span>
							<input type="date" data-date-field="previous_to_date" value="${this.escape_attribute(this.previous_to_date || defaultPreviousRange.to_date)}">
						</label>
					</div>
				</div>
		...
		$popover.find('.accounts-date-apply').on('click', () => {
			...
			if (!from_date || !to_date || !previous_from_date || !previous_to_date) {
