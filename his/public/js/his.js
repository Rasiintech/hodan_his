	...
	frappe.call({
		...
		callback: function (r) {
			...
			if (tab == "labs") {
				...
				else if (tab == "dental_plan") {
				this.table = new Tabulator("#dental_plan", {
					// layout:"fitDataFill",
					layout: "fitDataFill",
					//  layout:"fitColumns",
					// responsiveLayout:"collapse",
				...
				let row = this
					this.table.on("rowClick", function(e ,rows){
						...
						frappe.set_route("Form" ,"Dental Plan" ,  rows._row.data.sr)
					});
			}
			else {
				if (columns) {
...
function setup_datatable(columns, data, group) {
	...
	// this.table = new Tabulator("#history_", {
	// 	// layout:"fitDataFill",
	// 	layout: "fitDataFill",
	// 	//  layout:"fitColumns",
	// 	// responsiveLayout:"collapse",
...
function employee_statement(party) {
	let d = new frappe.ui.Dialog({
		title: 'Enter details',
		fields: [
			{
...
function payment_entry(party, paid_amount){
	frappe.call({
		method: "his.api.create_p_e.create_payment_entry",
		args: {
			party: party,
