...
def add_service_items(so, doc):
    items=[]
    for child_table in ("lab_test_prescription", "procedure_prescription" , "radiology_prescription", "therapies", "services_prescription", "packages_prescription"):
        for row in doc.get(child_table):
            ...
            if child_table=="procedure_prescription":
                so_item.sex = row.sexxx
                so_item.weight = row.weighttx
                so_item.diagnosis = row.diagnosissx
                so_item.procedure = row.procedure
                so_item.surgeonh = row.surgeonh
                so_item.proposed_date_of_surgury = row.proposed_date_of_surgury
                so_item.surgery_lenght_of_time = row.surgery_lenght_of_time

            if child_table=="services_prescription":
                so_item.qty = row.qty or 1
                so_item.rate = row.rate

            # Add comments only if attribute exists
            if hasattr(row, 'lab_test_comment'):
...
def get_item_and_is_billable(row):
    if row.doctype == "Lab Prescription":
        ...
        return frappe.get_cached_value(
            "Therapy Type", row.therapy_type, ("item", "is_billable")
        )
